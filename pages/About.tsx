...
import { Quote, User as UserIcon, MapPin, Camera, Upload, CheckCircle, Trash2, ShieldCheck, Loader2 } from 'lucide-react';
...
const About: React.FC = () => {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    alert("Profile published to Featured Farmers!");
  };

  const removeMyProfile = async () => {
    if (!user) return;
    if (!confirm("Are you sure you want to remove your community profile?")) return;
    
    setIsActionPending(true);
    await DatabaseService.deleteFeaturedFarmer(user.id);
    await fetchFarmers();
    setIsActionPending(false);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
       ...
       {/* Interview Section - Clean Centered Layout */}
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
             ...
             {user && (
               <div className="flex items-center gap-3">
                 {myProfile ? (
                   <Button variant="danger" onClick={removeMyProfile} disabled={isActionPending} className="flex items-center group">
                     {isActionPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Trash2 className="w-4 h-4 mr-2 group-hover:scale-110 transition-transform" />}
                     Delete Profile
                   </Button>
                 ) : (
               ...
               <form onSubmit={handleUploadSubmit} className="grid md:grid-cols-3 gap-8">
                 ...
                 <div className="md:col-span-2 flex flex-col">
                   ...
                   <textarea 
                     ...
                     placeholder="e.g. 'I am a wheat farmer from Punjab. This year, AgriFair helped me spot a 10% price mismatch, saving my season!'"
                     value={bio}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
               {featuredFarmers.length === 0 ? (
                 ...
                 featuredFarmers.map((farmer) => (
                   <div key={farmer.userId} className="group bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
